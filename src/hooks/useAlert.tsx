...
function combinarClasses(
  base: string,
  adicionais?: string | readonly string[],
) {
...
export const useSweetAlert = () => {
  const showAlert = useCallback(
    async ({
      ...
      ...opcoes
    }: OpcoesAlerta): Promise<SweetAlertResult> => {
      try {
        return await Swal.fire({
          ...
          customClass: {
            ...
            confirmButton: combinarClasses(
              obterClassesBotao(varianteConfirmacao),
              opcoes.customClass?.confirmButton,
            ),
            cancelButton: combinarClasses(
              obterClassesBotao("danger"),
              opcoes.customClass?.cancelButton,
            ),
            denyButton: combinarClasses(
              obterClassesBotao("danger"),
              opcoes.customClass?.denyButton,
            ),
          },
        });
      } catch (erro) {
