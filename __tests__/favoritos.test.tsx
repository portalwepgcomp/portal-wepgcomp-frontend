...
const favorites = [2025, 2026].map((year) => ({
  ...
  submission: {
    eventEditionId: `edition-${year}`,
    title: `Apresentação ${year}`,
  },
}));
const response = (items = favorites) => ({
  data: { bookmarkedPresentations: items },
});
  ...
  return (
    <QueryClientProvider client={client}>
      <AuthContext.Provider
        value={
          { user: userId ? { id: userId } : null, signed } as React.ContextType<
            typeof AuthContext
          >
        }
      >
        <EdicaoContext.Provider
          value={
            {
              loadingEdicao,
              Edicao: edition ? { id: edition } : null,
            } as React.ContextType<typeof EdicaoContext>
          }
        >
...
it("envia a edição e filtra a resposta da API atual", async () => {
  ...
  expect(get).toHaveBeenCalledWith(
    ...
    expect.objectContaining({ params: { eventEditionId: "edition-2025" } }),
  );
  expect(result.bookmarkedPresentations).toEqual([favorites[0]]);
});

it("não consulta favoritos sem edição, usuário ou sessão", async () => {
  expect(await presentationApi.getPresentationBookmarks("")).toEqual({
    bookmarkedPresentations: [],
  });
  edition = undefined;
  const { result, rerender } = renderHook(useListaFavoritos, {
    wrapper: Wrapper,
  });
...
it("remove o favorito uma vez e atualiza somente a consulta ativa", async () => {
  ...
  expect(remove).toHaveBeenCalledWith("/presentation/bookmark", {
    params: { presentationId: "presentation-2025" },
  });
  edition = "edition-2026";
  rerender();
  await waitFor(() => expect(result.current.itens).toEqual([favorites[1]]));
...
it("atualiza a lista quando uma apresentação é favoritada em outra tela", async () => {
  ...
  const { result } = renderHook(
    () => ({ list: useListaFavoritos(), actions: usePresentation() }),
    { wrapper: Wrapper },
  );
  await waitFor(() => expect(result.current.list.isLoading).toBe(false));
  get.mockResolvedValue(response());
  await act(async () =>
    result.current.actions.postPresentationBookmark({
      presentationId: "presentation-2025",
    }),
  );
  await waitFor(() =>
    expect(result.current.list.itens).toEqual([favorites[0]]),
  );
});
...
it("o card delega a remoção à listagem sem duplicar o DELETE", async () => {
  ...
  await waitFor(() =>
    expect(container.querySelector("div.cursor-pointer svg")).not.toBeNull(),
  );
